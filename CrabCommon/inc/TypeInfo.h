...
namespace static_type_info
{
    ...
    {
        return (str.size() == 0) ? value : hash_32_fnv1a_const(std::string_view(str.data() + 1, str.size() - 1), (value ^ uint32_t(str[0])) * prime_32_const);
    }

    inline constexpr uint64_t hash_64_fnv1a_const(std::string_view str,
                                                  const uint64_t   value = val_64_const) noexcept
    {
        return (str.size() == 0) ? value : hash_64_fnv1a_const(std::string_view(str.data() + 1, str.size() - 1), (value ^ uint64_t(str[0])) * prime_64_const);
    }
                                             ...
                                             const uint32_t   value = VAL_32_CONST) noexcept
{
    return str.empty() ?
               value :
               Hash32_FNV1A_const(std::string_view(str.data() + 1, str.size() - 1),
                                  (value ^ static_cast<uint32_t>(str[0])) * PRIME_32_CONST);
}
                                             ...
                                             const uint64_t   value = VAL_64_CONST) noexcept
{
    return (str.empty()) ? value :
                           Hash64_FNV1A_const(std::string_view(str.data() + 1, str.size() - 1),
                                              (value ^ static_cast<uint64_t>(str[0])) * PRIME_64_CONST);
}
...
namespace detail
{
    ...
    template<typename T>
    constexpr TypeName GetTypeName_Internal()
    {
...
namespace detail
{
    ...
    template<class T>
    constexpr TypeIndex GetTypeIndex_Internal()
    {
...
struct TypeInfo
{
    ...
    static TypeInfo Get()
    {
        return { detail::GetTypeName_Internal<Ty>(), detail::GetTypeIndex_Internal<Ty>() };
    }
};
...
struct hash<crab::TypeInfo>
{
    ...
    size_t                     operator()(const crab::TypeInfo& id) const noexcept { return hasher(id.index); }
};
