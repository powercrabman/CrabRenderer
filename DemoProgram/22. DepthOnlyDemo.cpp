...
void DepthOnlyDemo::OnEnter()
{
    // Panels
    m_sceneHierarchy = CreateScope<SceneHierarchy>();
}

void DepthOnlyDemo::OnExit()
{
}

void DepthOnlyDemo::OnUpdate(TimeStamp& in_ts)
{
...
void DepthOnlyDemo::_DrawMirror()
{
...
void DepthOnlyDemo::OnRender(TimeStamp& in_ts)
{
    auto* dx = D11_API;
    ...
    if (GetView<MirrorComponent>().size() > 0)
    {
...

        {
            ...
            auto cmrPos   = m_camera.GetTransform().position;
            auto cmrRot   = m_camera.GetTransform().rotate;
            Mat4 viewProj = m_camera.GetComponent<CameraComponent>()
                                .GetViewProj(cmrPos, cmrRot);
            ...
            cc.eyePosition = cmrPos;

            GetGlobalConstants().UpdateMirrorCamera(cc);
        }
        ...
        GetGlobalConstants().GetCameraConstant()->Bind(1, eShaderFlags_VertexPixelShader);
...
void DepthOnlyDemo::OnRenderGUI(TimeStamp& in_ts)
{
...
void DepthOnlyDemo::OnEvent(CrabEvent& in_event)
{
}

void DepthOnlyDemo::_RenderBasicMesh()
{
    // Common Mesh
...
void DepthOnlyDemo::_RenderSkybox()
{
    // Environment Map
...
void DepthOnlyDemo::_RenderPBRMesh(TransformComponent& t, MeshRenderer& m, PBRMaterialComponent& mat)
{
    TransformConstant tc = {};
    tc.world             = t.GetWorld();
    tc.worldInvTranspose = tc.world.Invert().Transpose();
...
void DepthOnlyDemo::_BuildPostProcess()
{
    ...
    auto postEffect = ImageFilterFactory::CreatePostEffect(width, height, {});
