...
void MousePickingDemo::Init()
{
...

    {
        ...
        std::filesystem::path root = R"(Resources\MousePickingDemo)";
    ...
    // Primitive
    {
        ...
        // Torus
        {
            Entity e   = CreateEntity();
            auto&  t   = e.GetComponent<Transform>();
            t.position = Vec3(15.f, 7.f, 20.f);

            Vec3 rotate = Vec3::Zero;
            rotate.x    = 45.f * DEG2RAD;
            rotate.z    = 45.f * DEG2RAD;
            t.quaternion = QuatFromEuler(rotate);

            auto& mc = e.AddComponent<MeshComponent>();
            mc       = commonMC;
            mc.mesh  = MeshFactory3D::CreateTorus(1.f, 0.5f, 32, 32);

            auto& mat     = e.AddComponent<MaterialComponent>();
            mat.ambient   = Vec4(1.f, 1.f, 1.f, 1.f);
            mat.diffuse   = Vec4(0.1f, 0.8f, 0.1f, 1.f);
            mat.specular  = Vec4(1.f, 1.f, 1.f, 1.f);
            mat.shininess = 1.f;

            e.AddComponent<TagComponent>("Torus");
            auto& obb     = e.AddComponent<OBBComponent>();
            obb.size.x    = 2.f;
            obb.size.y    = 0.5f;
            obb.size.z    = 2.f;
            obb.center    = Vec3::Zero;
            obb.debugMesh = MeshFactory3D::CreateCube(Vec3 { 2.f, 0.5f, 2.f });
        }
    }
    ...
    //===================================================
    {
        m_cbPixelShaderData.diffuseMapStrength  = 0.35f;
        m_cbPixelShaderData.specularMapStrength = 0.65f;
        m_cbPixelShaderData.speculerShininess   = 1.5f;
...
void MousePickingDemo::OnEnter()
{
}

void MousePickingDemo::OnExit()
{
    GetAppWindow().EnableMouseRelativeMode(false);
}

void MousePickingDemo::OnUpdate(TimeStamp& in_ts)
{
    ...
    // Mouse Picking
    {
        ...
        {
            auto [x, y] = Input::GetMousePos();
            auto& cmr   = m_camera.GetComponent<CameraControllerComponent>();
            _MousePicking(x, y);
        }
    }
...
void MousePickingDemo::OnRender(TimeStamp& in_ts)
{
    auto* dx = D11_API;
    m_defaultDSS->Bind();
...
void MousePickingDemo::OnRenderGUI(TimeStamp& in_ts)
{
    ...
    ImGui::Begin("Snapshot", nullptr, ImGuiWindowFlags_AlwaysAutoResize);

    ImGui::InputText("File Name", m_snapshotFileName, sizeof(m_snapshotFileName));

    auto [width, height] = GetAppWindow().GetWindowSize();

    float btnWidth = ImGui::GetContentRegionAvail().x;
    if (ImGui::Button("Take FullScreen Snapshot", { btnWidth, 0.f }))
    {
        D11_Snapshot snapshot;
        snapshot.Capture(D11_API->GetSwapChain()->GetBackBuffer()->GetRenderTarget());
        snapshot.Save(m_snapshotFileName);
    }

    ImGui::End();

    ImGui::Begin("Mouse Picking");
...
void MousePickingDemo::OnEvent(CrabEvent& in_event)
{
}

void MousePickingDemo::_DrawGUI()
{
    ...
    Vec3 rotate = trans.quaternion.ToEuler() * RAD2DEG;
    ImGui::Text("Pitch: %.2f deg", rotate.x);
    ImGui::Text("Yaw: %.2f deg", rotate.y);
    ImGui::Text("Roll: %.2f deg", rotate.z);
...
void MousePickingDemo::_BuildPostProcessData()
{
    m_ppStack.clear();
...
void MousePickingDemo::_MousePicking(uint32 in_x, uint32 in_y)
{
    auto [width, height] = GetAppWindow().GetWindowSize();
    ...
    auto& cmr = m_camera.GetComponent<CameraControllerComponent>().camera;
    auto& t   = m_camera.GetComponent<Transform>();
    ...
    GetView<OBBComponent, Transform, IDComponent>().each(
            ...
            IDComponent&  in_id)
        {
            ...
            if (obb.Intersects(ray.position, ray.direction, dist))
            {
                ...
                {
                    shortestDist     = dist;
                    m_selectedEntity = FindEntity(in_id.id);
                }
            }
        });
    ...
    CRAB_ASSERT(ray.Intersects(farZ, distTofarZ), "Ray does not intersect with far plane");

    if (shortestDist > distTofarZ)
        m_selectedEntity = Entity::s_null;

    if (m_selectedEntity.IsValid())
        Log::Debug("Selected Entity ID: {0}", m_selectedEntity.GetID().id);
    else
