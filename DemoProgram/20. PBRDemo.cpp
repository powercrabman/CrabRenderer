...
void PBRDemo::Init()
{
    // Pipeline
    {
        m_basicPipeline  = GetPipelineLibrary().GetPipeline("PBRState");
        m_skyboxPipeline = GetPipelineLibrary().GetPipeline("SkyboxState");
    }
...
void PBRDemo::OnRender(TimeStamp& in_ts)
{
    ...
    GetView<TransformComponent, LightComponent>().each(
        [](TransformComponent& t, LightComponent& l)
        {
            if (l.dirtyBit)
            {
                l.dirtyBit = false;
                ...
                lc.type           = static_cast<int>(l.type);
    ...
    // Common Mesh
    GetView<TransformComponent, MeshRenderer, PBRMaterialComponent>().each(
        [](TransformComponent& t, MeshRenderer& m, PBRMaterialComponent& mat)
        {
            TransformConstant tc = {};
            tc.world             = t.GetWorld();
            tc.worldInvTranspose = tc.world.Invert().Transpose();
...
void PBRDemo::_BuildPostProcess()
{
