...
void ModelDemo::Init()
{
    ...
    // Shader
    {
        ...
        m_blinnPhongPS = D11_PixelShader::CreateFromFile("BlinnPhongPixelShader2.hlsl", "main");
    ...
    // Model Entity
    {
        ...
        // Material Component
        PhongMaterial modelMat;
        modelMat.ambient   = Vec4(0.1f, 0.1f, 0.1f, 1.f);
        modelMat.diffuse   = Vec4(1.f, 1.f, 1.f, 1.f);
        modelMat.specular  = Vec4(1.f, 1.f, 1.f, 1.f);
        modelMat.shininess = 32.f;

        auto& mc      = m_modelEntity.AddComponent<MaterialComponent>(modelMat, true);
        mc.material   = modelMat;
        mc.useTexture = true;
    }

    // Light Entity
    {
        m_pointLightEntity = CreateEntity();

        auto& lc          = m_pointLightEntity.AddComponent<LightComponent>();
        ...
        auto& lt    = m_pointLightEntity.GetComponent<Transform>();
        lt.position = Vec3(2.f, 0.f, 0.f);

        auto& mr        = m_pointLightEntity.AddComponent<MeshRenderer>();
        ...
        auto& mat      = m_pointLightEntity.AddComponent<MaterialComponent>();
    ...
    // Spot Light
    {
        Entity spotLightEntity = CreateEntity();
        auto&  lc              = spotLightEntity.AddComponent<LightComponent>();
        lc.lightType           = eLightType::Spot;
        lc.lightColor          = Vec3(1.f, 1.f, 1.f);
        lc.lightIntensity      = 1.f;
        lc.falloffStart        = 1.f;
        lc.falloffEnd          = 50.f;
        ...
        auto& lt    = spotLightEntity.GetComponent<Transform>();
        lt.position = Vec3(0.f, 0.f, -2.f);
        lt.scale *= 0.01f;

        auto&           mr = spotLightEntity.AddComponent<ModelRenderer>();
        D11_ModelLoader loader;
        loader.Load(R"(Resources\flashlight\source\Flashlight.obj)");
        auto meshData   = loader.GetMeshDatas();
        mr.model        = loader.CreateModel();
        mr.topology     = eTopology::TriangleList;
        mr.pixelShader  = m_blinnPhongPS;
        mr.vertexShader = m_commonVS;

        auto& mat              = spotLightEntity.AddComponent<MaterialComponent>();
        mat.material.ambient   = Vec4(1.f, 1.f, 1.f, 1.f);
        mat.material.diffuse   = Vec4(1.f, 1.f, 1.f, 1.f);
        mat.material.specular  = Vec4(1.f, 1.f, 1.f, 1.f);
        mat.material.shininess = 32.f;
        mat.useTexture         = true;

        auto& nr        = spotLightEntity.AddComponent<NormalRenderer>();
...
void ModelDemo::OnExit()
{
}

void ModelDemo::OnUpdate(TimeStamp& in_ts)
{
...
void ModelDemo::OnRender(TimeStamp& in_ts)
{
    auto* dx = D11_API;
...
void ModelDemo::OnRenderGUI(TimeStamp& in_ts)
{
    ...
    // Point Light Entity
    ImGui::PushID("Point Light Entity");
    {
        ImGui::SeparatorText("Point Light");

        ImGui::BulletText("Transform");
        DrawTransformInspector(m_pointLightEntity.GetComponent<Transform>());

        ImGui::BulletText("Light Component");
        DrawLightComponentInspector(m_pointLightEntity.GetComponent<LightComponent>(), false);
    }
    ImGui::PopID();

    // Spot Light Entity
    ImGui::PushID("Spot Light Entity");
    {
        ImGui::SeparatorText("Spot Light");

        ImGui::BulletText("Transform");
        DrawTransformInspector(m_spotLightEntity.GetComponent<Transform>());

        ImGui::BulletText("Light Component");
        DrawLightComponentInspector(m_spotLightEntity.GetComponent<LightComponent>(), false);
    }
    ImGui::PopID();
...
void ModelDemo::OnEvent(CrabEvent& in_event)
{
}

void ModelDemo::_CreateRasterizerState()
{
