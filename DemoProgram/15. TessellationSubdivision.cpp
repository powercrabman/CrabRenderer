...
void TessellationSubdivision::Init()
{
    //===================================================
    //                    Texture
    //===================================================
    ...
    {
        m_commonSS = D11_SamplerState::Create(
...

    {
        ...
        m_cbCamera       = D11_ConstantBuffer<CB_Camera>::Create(1, eShaderFlags_VertexShader | eShaderFlags_PixelShader | eShaderFlags_HullShader | eShaderFlags_DomainShader);
        m_cbPixelShader  = D11_ConstantBuffer<CB_PixelShader>::Create(2, eShaderFlags_PixelShader);
        m_cbTessellation = D11_ConstantBuffer<CB_Tessellation>::Create(2, eShaderFlags_HullShader);
    }
...

    {
        ...
        m_tessVS = D11_VertexShader::CreateFromFile(
            "TDVS.hlsl", "main", elem);

        m_tessHS = D11_HullShader::CreateFromFile(
            "TDHS.hlsl", "main");

        m_tessDS = D11_DomainShader::CreateFromFile(
            "TDDS.hlsl", "main");

        m_tessPS = D11_PixelShader::CreateFromFile(
            "TDPS.hlsl", "main");
    }
    ...
    // Primitive
    {
        ...
        std::vector<TessDemoVertex> vertices = {
            TessDemoVertex { Vec3 { -1.f, 0.f, 0.f } },   // left
            TessDemoVertex { Vec3 { 0.f, 1.f, 0.f } },    // top
            TessDemoVertex { Vec3 { 1.f, 0.f, 0.f } },    // right

            TessDemoVertex { Vec3 { -1.f, 0.f, 0.f } },   // left
            TessDemoVertex { Vec3 { 1.f, 0.f, 0.f } },    // right
            TessDemoVertex { Vec3 { 0.f, -1.f, 0.f } }    // bottom
        };
        ...
        auto& mc = e.AddComponent<TessellationComponent>();
        mc.constantBuffers.push_back(m_cbObject);
        mc.constantBuffers.push_back(m_cbTessellation);
        mc.constantBuffers.push_back(m_cbCamera);
        mc.vertexShader = m_tessVS;
        mc.hullShader   = m_tessHS;
        mc.domainShader = m_tessDS;
        mc.pixelShader  = m_tessPS;
        mc.topology     = eTopology::PatchList_3;
        mc.vertices     = D11_VertexBuffer::Create(vertices);
    }
...
void TessellationSubdivision::OnEnter()
{
}

void TessellationSubdivision::OnExit()
{
    GetAppWindow().EnableMouseRelativeMode(false);
}

void TessellationSubdivision::OnUpdate(TimeStamp& in_ts)
{
...
void TessellationSubdivision::OnRender(TimeStamp& in_ts)
{
    auto* dx = D11_API;
    ...
    GetView<TessellationComponent, Transform>().each(
        [&](TessellationComponent& in_tc,
            Transform&             in_t)
        {
            CB_Object data;
            data.world             = in_t.GetWorld();
            ...
            for (const auto& cBuffer: in_tc.constantBuffers)
                cBuffer->Bind();

            for (uint32 i = 0; i < in_tc.textures.size(); ++i)
                in_tc.textures[i]->Bind(i, eShaderFlags_PixelShader);

            for (uint32 i = 0; i < in_tc.samplers.size(); ++i)
                in_tc.samplers[i]->Bind(i, eShaderFlags_PixelShader);

            dx->SetTopology(in_tc.topology);
            in_tc.vertexShader->Bind();
            in_tc.pixelShader->Bind();
            in_tc.hullShader->Bind();
            in_tc.domainShader->Bind();

            if (in_tc.geometryShader)
                in_tc.geometryShader->Bind();
            else
                dx->SetGeometryShader(nullptr);

            in_tc.vertices->Bind();
            dx->Draw(in_tc.vertices->GetVertexCount());

            in_tc.hullShader->Unbind();
            in_tc.domainShader->Unbind();
        });
    ...
    if (m_tessellationDirty)
    {
        CB_Tessellation tess;
        tess.edgeTessFactor.x   = m_tessFactor[0];
        tess.edgeTessFactor.y   = m_tessFactor[1];
        tess.edgeTessFactor.z   = m_tessFactor[2];
        tess.edgeTessFactor.w   = m_tessFactor[3];
        tess.insideTessFactor.x = insideTessFactor[0];
        tess.insideTessFactor.y = insideTessFactor[1];
        m_cbTessellation->UpdateData(tess);
        m_tessellationDirty = false;
    }
...
void TessellationSubdivision::OnRenderGUI(TimeStamp& in_ts)
{
    ...
    ImGui::Begin("Tessellation", nullptr, ImGuiWindowFlags_AlwaysAutoResize);

    if (ImGui::DragInt4("Tessellation Factor", m_tessFactor, 0.25f, 1.f, 64.f))
        m_tessellationDirty = true;

    if (ImGui::DragInt2("Inside Tessellation Factor", insideTessFactor, 0.25f, 1.f, 64.f))
        m_tessellationDirty = true;

    ImGui::End();
}

void TessellationSubdivision::OnEvent(CrabEvent& in_event)
{
}

void TessellationSubdivision::_DrawGUI()
{
...
void TessellationSubdivision::_BuildPostProcessData()
{
    m_ppStack.clear();
    m_thresholdPP.reset();
    m_combinePP.reset();
