    ...
    eTextureLoadFlags            in_flags)
{
    ...
    if (ext == ".dds")
    {
        loadData.succeed = d3d::CheckOK(LoadFromDDSFile(
                                            in_path.c_str(),
                                            DDS_FLAGS_NONE,
                                            &loadData.metadata,
    ...
    {
        loadData.succeed = d3d::CheckOK(LoadFromEXRFile(
                                            in_path.c_str(),
                                            &loadData.metadata,
                                            loadData.scratchImage),
    ...
    else
    {
        ...
        loadData.succeed = d3d::CheckOK(LoadFromWICFile(
                                            in_path.c_str(),
                                            flags,
                                            &loadData.metadata,
                                            loadData.scratchImage),
...
void TextureLoader::GenerateMipmap(TextureLoadData& inout_data)
{
    ...
    if (inout_data.succeed)
    {
        ...
        if (d3d::CheckOK(GenerateMipMaps(
...
void TextureLoader::GenerateCubeMap(TextureLoadData& inout_data)
{
    ...
    if (d3d::CheckOK(cubeMap.InitializeCubeFromImages(images, 6),
