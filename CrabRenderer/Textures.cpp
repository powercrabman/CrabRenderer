...
void ShaderResourceBuffer::Init(ID3D11Buffer* in_buffer)
{
    m_srv = ID3D11ShaderResourceViewUtil::CreateSRV(in_buffer, eFormat::Unknown);
...
void Texture2D::Init(ID3D11Texture2D* in_texture, eFormat in_format)
{
    ...
    m_srv              = ID3D11ShaderResourceViewUtil::CreateSRV(in_texture, in_format);
                       ...
                       eFormat                                in_format)
{
    ...
    ComPtr<ID3D11Texture2D> tex = ID3D11Texture2DUtil::CreateTexture2DArray(
        textures,
...
void TextureCube::Init(ID3D11Texture2D* in_textureCube, eFormat in_format)
{
    ...
    m_srv              = ID3D11ShaderResourceViewUtil::CreateSRV(in_textureCube, in_format);
    m_width            = texDesc.Width;
    m_height           = texDesc.Height;
    m_format           = in_format;
    m_mipMapLevelCount = texDesc.MipLevels;
...
void Texture2DArray::Init(ID3D11Texture2D* in_textureArray, eFormat in_format)
{
    ...
    m_srv          = ID3D11ShaderResourceViewUtil::CreateSRV(in_textureArray, in_format);
    m_width        = desc.Width;
...
{
    auto tex = ID3D11Texture2DUtil::CreateTexture2DArray(in_textures, eTextureCreationFlags_None);
    ...
    bool    in_cpuWrite)
{
    ...
    ComPtr<ID3D11Texture2D>
        tex = ID3D11Texture2DUtil::CreateTexture2D(
            in_width,
            in_height,
            in_format,
            D3D11_USAGE_STAGING,
