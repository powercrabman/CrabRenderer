#include "InternalPCH.h"

#include "Snapshot.h"

#include "Renderer.h"
#include "RenderTarget.h"

#include <DirectXTex.h>

namespace crab
{
//
//Snapshot::Snapshot()
//{
//}
//
//Snapshot::~Snapshot()
//{
//}
//
//void Snapshot::Capture(const Ref<RenderTarget>& in_rt)
//{
//    auto dx  = GetRenderer().GetDevice();
//    auto ctx = GetRenderer().GetDeviceContext();
//
//    ComPtr<ID3D11Resource> res;
//    in_rt->GetRTV()->GetResource(res.GetAddressOf());
//
//    ComPtr<ID3D11Texture2D> tex;
//    d3d11::CheckOK(res.As(&tex), "As Fail.");
//
//    D3D11_TEXTURE2D_DESC desc = {};
//    tex->GetDesc(&desc);
//
//    D3D11_TEXTURE2D_DESC captureDesc = desc;
//    captureDesc.BindFlags            = 0;
//    captureDesc.CPUAccessFlags       = D3D11_CPU_ACCESS_READ;
//    captureDesc.Usage                = D3D11_USAGE_STAGING;
//    m_format                         = desc.Format;
//
//    d3d11::CheckOK(dx->CreateTexture2D(
//                         &captureDesc,
//                         nullptr,
//                         m_texture.GetAddressOf()),
//                     "CreateSRV Fail.");
//
//    ctx->CopyResource(m_texture.Get(), tex.Get());
//    m_width  = desc.Width;
//    m_height = desc.Height;
//    log::Info("Snapshot captured: {0}x{1}", m_width, m_height);
//}
//
//void Snapshot::Save(const std::filesystem::path& in_path)
//{
//    if (m_texture == nullptr)
//    {
//        DEBUG_BREAK("Texture is nullptr.");
//        return;
//    }
//
//    auto ctx = GetRenderer().GetDeviceContext();
//
//    D3D11_MAPPED_SUBRESOURCE mapped = {};
//    d3d11::CheckOK(ctx->Map(m_texture.Get(), 0, D3D11_MAP_READ, 0, &mapped), "Map Fail.");
//
//    DirectX::ScratchImage image;
//
//    D3D11_TEXTURE2D_DESC desc = {};
//    m_texture->GetDesc(&desc);
//    d3d11::CheckOK(image.Initialize2D(m_format,
//                                        desc.Width,
//                                        desc.Height,
//                                        1,
//                                        1),
//                     "Initialize2D Fail.");
//
//    uint8_t* dest       = image.GetPixels();
//    uint8_t* src        = (uint8_t*)mapped.pData;
//    size_t   rowPitch   = mapped.RowPitch;
//    size_t   slicePitch = rowPitch * desc.Height;
//
//    for (size_t i = 0; i < desc.Height; i++)
//    {
//        memcpy(dest, src, rowPitch);
//        dest += rowPitch;
//        src += rowPitch;
//    }
//
//    ctx->Unmap(m_texture.Get(), 0);
//
//    std::filesystem::path fileName = in_path.stem();
//    std::filesystem::path ext      = in_path.extension();
//
//    DirectX::WICCodecs codec = {};
//
//    if (ext == ".jpg" || ext == ".jpeg")
//        codec = DirectX::WIC_CODEC_JPEG;
//    else if (ext == ".bmp")
//        codec = DirectX::WIC_CODEC_BMP;
//    else if (ext == ".tiff")
//        codec = DirectX::WIC_CODEC_TIFF;
//    else if (ext == ".gif")
//        codec = DirectX::WIC_CODEC_GIF;
//    else if (ext == ".png" || ext.empty())
//        codec = DirectX::WIC_CODEC_PNG;
//    else
//    {
//        DEBUG_BREAK("Invalid extension.");
//        return;
//    }
//
//    d3d11::CheckOK(DirectX::SaveToWICFile(image.GetImages(),
//                                            image.GetImageCount(),
//                                            DirectX::WIC_FLAGS_NONE,
//                                            DirectX::GetWICCodec(codec),
//                                            in_path.c_str()),
//                     "SaveToWICFile Fail.");
//
//    log::Info("Snapshot saved: {0}", in_path.string());
//}
//
//Vec4 Snapshot::GetPixelColorFloat(const Int2& in_pos) const
//{
//    if (m_texture == nullptr)
//    {
//        DEBUG_BREAK("Texture is nullptr.");
//        return Vec4::Zero;
//    }
//
//    if (in_pos.x < 0 || in_pos.y < 0)
//    {
//        DEBUG_BREAK("Invalid x, y.");
//        return Vec4::Zero;
//    }
//
//    auto ctx = GetRenderer().GetDeviceContext();
//
//    D3D11_MAPPED_SUBRESOURCE mapped = {};
//    d3d11::CheckOK(ctx->Map(m_texture.Get(),
//                              0,
//                              D3D11_MAP_READ,
//                              0,
//                              &mapped),
//                     "Map Fail.");
//
//    uint8_t* src      = static_cast<uint8_t*>(mapped.pData);
//    size_t   rowPitch = mapped.RowPitch;
//
//    uint8_t* pixel = src + in_pos.y * rowPitch + in_pos.x * 4;
//
//    Vec4 color;
//    color.x = pixel[0] / 255.f;
//    color.y = pixel[1] / 255.f;
//    color.z = pixel[2] / 255.f;
//    color.w = pixel[3] / 255.f;
//
//    ctx->Unmap(m_texture.Get(), 0);
//
//    return color;
//}
//
//Int4 Snapshot::GetPixelColorUInt(const Int2& in_pos) const
//{
//    if (m_texture == nullptr)
//    {
//        DEBUG_BREAK("Texture is nullptr.");
//        return Int4 { 0, 0, 0, 0 };
//    }
//    if (in_pos.x < 0 || in_pos.y < 0)
//    {
//        DEBUG_BREAK("Invalid x, y.");
//        return Int4 { 0, 0, 0, 0 };
//    }
//    auto                     ctx    = GetRenderer().GetDeviceContext();
//    D3D11_MAPPED_SUBRESOURCE mapped = {};
//    d3d11::CheckOK(ctx->Map(m_texture.Get(),
//                              0,
//                              D3D11_MAP_READ,
//                              0,
//                              &mapped),
//                     "Map Fail.");
//    uint8_t* src      = static_cast<uint8_t*>(mapped.pData);
//    size_t   rowPitch = mapped.RowPitch;
//    uint8_t* pixel    = src + in_pos.y * rowPitch + in_pos.x * 4;
//    Int4     color;
//    color.x = pixel[0];
//    color.y = pixel[1];
//    color.z = pixel[2];
//    color.w = pixel[3];
//    ctx->Unmap(m_texture.Get(), 0);
//    return color;
//}

}   // namespace crab