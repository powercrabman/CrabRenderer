...
float4 main(PSInput input) : SV_TARGET
{
    ...
    float4 finalColor = float4(0.0f, 0.0f, 0.0f, 1.f);
    
    finalColor += ComputeDirLight(g_dirLight, input.posW, E, input.normalW);
    finalColor += ComputePointLight(g_pointLight, input.posW, E, input.normalW);
    finalColor += ComputeSpotLight(g_spotLight, input.posW, E, input.normalW);
    finalColor = saturate(finalColor);
    
    float4 texColor = g_texture.Sample(g_sampler, input.texCoord) * g_useTexture
                    + float4(1.f, 1.f, 1.f, 1.f) * (1.0f - g_useTexture);
    
    finalColor = smoothstep(0.0f, 1.0f, finalColor) * g_useSmoothStep
               + finalColor * (1.0f - g_useSmoothStep);
