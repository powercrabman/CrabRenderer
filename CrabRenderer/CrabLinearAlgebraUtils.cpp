...
{
    CRAB_ASSERT(std::ranges::is_sorted(in_cascadeRanges), "Cascade ranges must be sorted.");

    std::vector<Frustum> cascadeFrustums = FrustumUtil::DivideCascade(in_viewFrustum, in_cascadeRanges);
    CRAB_ASSERT(cascadeFrustums.size() == MAX_CASCADE_SHADOW_LEVEL, "Cascade frustums size is not correct");
    ...
    float cascadeMinZ = CRAB_FLOAT_MAX;
    float cascadeMaxZ = CRAB_FLOAT_MIN;
    ...
    for (size_t i = 0; i < MAX_CASCADE_SHADOW_LEVEL; ++i)
    {
        ...
        float minX, maxX, minY, maxY;
        minX = minY = CRAB_FLOAT_MAX;
        maxX = maxY = CRAB_FLOAT_MIN;
